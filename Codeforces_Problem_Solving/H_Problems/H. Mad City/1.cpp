...
bool solve(){
    ...
    if(a == b) {no return true ;}
    if(mp[b]) {yes  return true ;}
    int found = -1 ;
    queue<int > q ;
    q.push(b) ;
    vis.clear() ;
    ...
    // cout << found << " " << distance << " " << dist2[found] << nl ;
    if(dist[found] < dist2[found]){
        yes
    }
    else{
        no
