...
bool solve(){
    ...
    ans += s[p];
    for (int i = p + 1; i < n; i++) {
        while (x > 0 && ans.size() > 1 && s[i] < ans.back()) {
            ans.pop_back();
            x--;
        }
        ans += s[i];
    ...
    // for(int i = 0 ; i < n ; ++i){
    //     int t = s[i] - '0' ;
    //     while(!st.empty() and st.top() > t and k < x){
    //         if(t == 0 and st.size() == 1) break ;
    //         st.pop() ;
    //         k += 1 ;
    //     }
    //     while(!st.empty() and st.top() == 0 and k < x - 1){
    //         if(st.size() > 1){
    //             int sees = st.top() ; st.pop() ;
    //             if(t < st.top() and t != 0){
    //                 st.pop() ;
    //                 k += 2 ;
    //             }
...
//#endif 
    ...
    while (t--){
        ...
        // tt += 1 ;
        // if(tt==495){
        //     string s;cin>>s;int x;cin>>x;cout<<s<<" "<<x<<nl;
        // }
        if(solve()){
            //count++ ;
        }else{
