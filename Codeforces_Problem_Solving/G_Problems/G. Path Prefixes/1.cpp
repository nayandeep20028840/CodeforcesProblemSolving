...
vector<array<int,3 > > adj[100005]; // Assuming maximum size
vector<int > ans(100005) ;
void dfs(int child, int parent, int a1, vector<int>& bb) {
    for(auto &vec : adj[child]){
        a1 += vec[1] ;
        if(bb.size()){
            bb.pb(bb.back() + vec[2]) ;
        }
        ...
        // cout << bb.size() << " " ;
        auto it = upper_bound(all(bb) , a1) - bb.begin() ;
        // cout << it << " " ;
        // if(it != 0){
        //     it -= 1 ;
        //     cout << it << " " << vec[0] << nl ;
...
bool solve() {
    ...
    // Clearing the adjacency list vector
    for(int i = 0; i <= n; ++i) {
        adj[i].clear();
    }
    ans.clear() ;
