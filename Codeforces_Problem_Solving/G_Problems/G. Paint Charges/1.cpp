...
bool solve(){
    ...
    function<int(int,int,int)> func=[&](int i,int j,int k)->int{
        ...
        // left side 
        int left = j ;
        if(left <= i and left + a[i] > i) {
            ans = min(ans , 1 + func(i + 1 , max(i + 1 , k) , k)) ;
        }
        // right side
        // cout << ans << " " ;
        int right = i ;
        right = min(n , right + a[i]) ;
        // if(right >= j and j < n) 
        if(j == i) ans = min(ans , 1 + func(i + 1 , right , right)) ;
        else ans = min(ans , 1 + func(i + 1 , j , right)) ;
        // not used
        if(j == i) ans = min(ans , func(i + 1 , max(j , k) , k)) ;
        else ans = min(ans , func(i + 1 , j , k)) ;
        return dp[i][j][k] = ans ;
    };
    cout << func(0 , 0 , 0) << nl ;
    return true ;
