...
bool solve(){
    ...
    int extra = 0 ;
    int f = 0 ;
    function<void(int , int , int) > dfs = [&](int child , int parent , int start) -> void {
        for(auto &i : adj[child]){
            if(i == parent || adj[i].size() != 2) continue ;
            // cout << i << " " << child << " " << parent << " " << vis[i] << nl ;
            if(vis[i] == 1 && i == start && adj[i].size() == 2 && f == 0) {
                extra += 1 ;
                f = 1 ;
                continue ;
            }
