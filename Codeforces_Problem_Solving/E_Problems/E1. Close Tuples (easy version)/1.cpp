...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        ...
        // cout << mp2[ft] << nl ;
        mp2[v[i]] -= 1 ;
        ans += (mp2[(v[i])] * (mp2[(v[i])] - 1))  / 2 ;
        // cout << ans << " " ;
        ans += (mp2[v[i]]) * (mp2[v[i] + 1]) ;
        ans += (mp2[v[i]]) * (mp2[v[i] + 2]) ;
        ans += (mp2[v[i]]) * (mp2[v[i] - 1]) ;
        ans += (mp2[v[i]]) * (mp2[v[i] - 2]) ;
        ans += (mp2[(v[i] + 1)] * (mp2[(v[i] + 1)] - 1))  / 2 ;
        // cout << ans << " " ;
        ans += (mp2[(v[i] + 2)] * (mp2[(v[i] + 2)] - 1))  / 2 ;
        // cout << ans << " " ;
        ans += (mp2[v[i] + 1]) * (mp2[v[i] + 2]) ;
        // cout << ans << " " ;
        ans += (mp2[(v[i] - 1)] * (mp2[(v[i] - 1)] - 1))  / 2 ; ;
        // cout << ans << " " ;
        ans += (mp2[(v[i] - 2)] * (mp2[(v[i] - 2)] - 1))  / 2 ;
