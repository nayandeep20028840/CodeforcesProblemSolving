...
bool solve(){
    int n , q ;
    cin >> n >> q ;
    vector<vector<int > > mat(3011 , vector<int > (3011 , 0)) ;
    for(int i = 0 ; i < n ; ++i){
        int h , w ;
        cin >> h >> w ;
        // cout << h << " " << w << nl ;
        // cout << h * w << nl ;
        for(int j = w ; j <= 3000 ; ++j){
            mat[h][j] += (h * w) ;
        }
    }
    // for(int i = 0 ; i <= 30 ; ++i){
    ...
    // cout << nl ;
    while(q--){
        int small , small1 , large , large1 ;
        cin >> small >> small1 >> large >> large1 ;
        int ans = 0 ;
        for(int i = small + 1 ; i <= large - 1 ; ++i){
            // cout << mat[i][large1 - 1] << " " << mat[i][small1] << nl ;
            ans += (mat[i][large1 - 1] - mat[i][small1]) ;
        }
        cout << ans << nl ;
    }
    return true ;
