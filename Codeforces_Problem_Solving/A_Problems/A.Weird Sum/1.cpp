...
bool solve(){
    ...
    for(auto &i : p){
        ...
        if(n > 0){
            for(int j = 0 ; j < n - 1 ; ++j){
                // if((((n - (j + 2)) * j) + (n - (j + 2))))
                ans += (pp[j + 1].F - pp[j].F) * (int)(j + 1) * (int)(n - (j + 1)) ;
                // else
                // ans += (pp[j + 1].F - pp[j].F) * (n - 1) ;
                // if((pp[j + 1].F - pp[j].F))
                // ans *= (((n - (j + 2)) * j) + (n - (j + 2))) ;
            }
        }
        sort(all(pp) , [&](pair<int,int> a, pair<int,int> b){ return a.second < b.second; }) ;
        if(n > 0){
            for(int j = 0 ; j < n - 1 ; ++j){
                // if((((n - (j + 2)) * j) + (n - (j + 2))))
                ans += (pp[j + 1].S - pp[j].S) * (int)(j + 1) * (int)(n - (j + 1)) ;
                // else 
                // ans += (pp[j + 1].S - pp[j].S) * (n - 1) ;
                // if((pp[j + 1].S - pp[j].S))
                // ans *= (((n - (j + 2)) * j) + (n - (j + 2))) ;
            }
        }
