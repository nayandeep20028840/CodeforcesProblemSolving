...
void dfs(int child , int parent) {
    ...
    for(auto &i : adj[child]) {
        ...
        dfs(i , child) ;
        dp[child][0] += max(dp[i][0] + abs(L[i] - L[child]), dp[i][1] + abs(R[i] - L[child])); // dp[][0] tells that we are taking left parent(child) value and trying subtracting both left and right child
        dp[child][1] += max(dp[i][0] + abs(L[i] - R[child]), dp[i][1] + abs(R[i] - R[child])); // dp[][1] tells that we are taking right parent(child) value and trying subtracting both left and right child 
    }
}
...
bool solve(){
    ...
    for(int i = 1 ; i < n + 1 ; ++i) { // we are clearing this vector of vector because we use a vector.
        adj[i].clear();    // if we haven't used vector<int > then we do need to clear the array.
    }
    return true ;
}
