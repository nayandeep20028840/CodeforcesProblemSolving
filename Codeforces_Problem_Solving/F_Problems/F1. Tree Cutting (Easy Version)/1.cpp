...
bool solve(){
    ...
    //     //     break ;
    //     // }
    //     if(v[i] == 2 && adj[i + 1].size() == 1){
    //         start = i + 1 ;
    //         break ;
    //     }
    //     if(v[i] == 1 && adj[i + 1].size() == 1){
    //         start = i + 1 ;
    //         // break ;
    //     }
    //     if(v[i] == 0 && adj[i + 1].size() == 1){
    //         start = i + 1 ;
    //         // break ;
    //     }
    // }
    ...
    function<void(int,int,int,int) > dfs = [&](int child , int parent , int red , int blue) -> void {
        RED[child] = (v[child - 1] == 1) ;
        BLUE[child] = (v[child - 1] == 2) ;
        for(auto &i : adj[child]){
            if(i == parent) continue ;
            dfs(i , child , red , blue) ;
            RED[child] += RED[i] ;
            BLUE[child] += BLUE[i] ;
        }
        if(parent){
            if(RED[child] == totalred && BLUE[child] == 0){
                ans += 1 ;
            }
            else if(BLUE[child] == totalblue && RED[child] == 0){
                ans += 1 ;
            }
        }
    };
    // cout << start << nl ;
    // if(v[start - 1] == 1) dfs(start , 0 , 1 , 0) ;
    // else if(v[start - 1] == 2) dfs(start , 0 , 0 , 1) ;
    // else dfs(start , 0 , 0 , 0) ;
    dfs(1 , 0 , 0 , 0) ;
    cout << ans << nl ;
    return true ;
}
