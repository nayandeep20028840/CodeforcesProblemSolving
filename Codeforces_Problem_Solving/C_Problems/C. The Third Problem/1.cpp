...
bool solve(){
    ...
    sort(all(p)) ;
    int mn = min(p[0].S , p[1].S) ;
    int mx = max(p[0].S , p[1].S) ;
    int ans = 1 ;
    for(int i = 2 ; i < n ; ++i){
        // cout << mx << " " << mn << " " << ans << nl ;
        if(p[i].S < mn){
            // ans = mod_MULTIPLICATION(ans , max(1ll , mx - p[i].S + 1 - i - 1) , MOD) ;
        }
        else if(p[i].S > mx){
            // ans = mod_MULTIPLICATION(ans , max(1ll , p[i].S - mn + 1 - i - 1) , MOD) ;
        }
        else{
            ans = mod_MULTIPLICATION(ans , max(1ll , mx - mn + 1 - i) , MOD) ;
        }
        mn = min(mn , p[i].S) ;
        mx = max(mx , p[i].S) ;
    }
