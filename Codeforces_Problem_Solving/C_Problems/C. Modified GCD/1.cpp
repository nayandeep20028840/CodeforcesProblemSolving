...
bool solve(){
    ...
    int gcd = GCD(a , b) ;
    vector<int > divisors ;
    for(int i = 1 ; i <= sqrtl(gcd) ; ++i){
        if(gcd % i == 0){
            divisors.pb(i) ;
            if(gcd % (gcd / i) == 0 and i != (gcd / i)){
                divisors.pb(gcd / i) ;
            }
        }
    }
    ...
    while(n--){
        int x , y ;
        cin >> x >> y ;
        auto start = lower_bound(all(divisors) , x) - divisors.begin() ;
        auto end = upper_bound(all(divisors) , y) - divisors.begin() ;
        if(start == divisors.size()){
            cout << -1 << nl ;
        }
        else{
            if(start == end) cout << -1 << nl ;
            else cout << divisors[end - 1] << nl ;
        }
    }
    return true ;
}
