...
struct SGT {
    ...
    vector<int> segtree ;
        SGT(int n) {
            ...
            sz = n ;
            segtree.resize(2 * sz , 1e9) ;
        }
        void output(int node){
            cout << segtree[node] << " " ;
        }
        void build(int node , int start , int end , vector<int > &arr) { // require the input array
            if (start == end) {
        ...
        int find(int node , int start , int end , int l , int r , int mn) {
            ...
            int mid = (start + end) / 2 ;
            if(segtree[node] == mn){
                cnt += find(2 * node , start , mid , l , r , mn) ;
                cnt += find(2 * node + 1 , mid + 1 , end , l , r , mn) ;
            }
            return cnt ;
        }
...
bool solve(){
    ...
    while(m--){
        ...
        if(num == 1){
            int index , value ;
            cin >> index >> value ;
            t.update(1 , 0 , n - 1 , index , value) ;
        }else{
            int l , r ;
            cin >> l >> r ;
            int ans = t.query(1 , 0 , n - 1 , l , r - 1) ;
            cout << ans << " " ;
            int answer = t.find(1 , 0 , n - 1 , l , r - 1 , ans) ;
            cout << answer << nl ;
        }
    }
    return true;
