...
bool solve(){
    ...
    for(int i = 1 ; i <= n ; ++i){
        auto it = ub(pf.begin() , pf.end() , a[i - 1] + pf[i - 1]) - (pf.begin()) ;
        // cout << it << " " << i - 1 << " h "  ;
        if(it != i - 1) {
            partial[i - 1] += 1 ;
            if(it > n) partial[n] -= 1 ;
            else partial[it - 1] -= 1 ;
            if(it <= n) random[it - 1] += abs(b[it - 1] -  abs(pf[it] - pf[i - 1] - a[i - 1])) ;
        }else{
            if(it <= n) random[it - 1] += abs(b[it - 1] -  abs(pf[it] - pf[i - 1] - a[i - 1])) ;
        }
        // check1(partial)
