...
bool solve(){
    int n ;
    cin >> n ;
    vector<int> x(n + 2 , 9999999999999) , h(n + 2 , 9999999999999) ;
    for(int i = 0 ; i < n ; ++i){
        cin >> x[i] >> h[i] ;
    }
    int dp[n + 1][3] = {0};
    dp[1][0] = 1 ;
    if(x[1] > x[0] + h[0]) dp[1][2] = 1 ;
    else dp[1][2] = -1;
    for(int i = 2 ; i <= n ; ++i){
        dp[i][1] = max({dp[i - 1][0] , dp[i - 1][1] , dp[i - 1][2]}) ;
        if(x[i - 2] + h[i - 2] < x[i - 1]){
            dp[i][0] = max({dp[i - 1][0] + 1 , dp[i - 1][1] + 1}) ;
        
            if(x[i - 2] - h[i - 2] > x[i - 1] + h[i - 1]){
        ...
        if(x[i] > x[i - 1] + h[i - 1]){
            dp[i][2] = max({dp[i - 1][0] + 1 , dp[i - 1][1] + 1 , dp[i - 1][2] + 1}) ;
        }else{
            dp[i][2] = -1;
        }
    }
