...
void init() {
    ...
    iF[N - 1] = fast(F[N - 1] , modg - 2) ; // it finds the inverse of F[N - 1] 
    for (int i = N - 2 ; i >= 0 ; --i)
...
int NCR(int n, int r) {
    if (r > n || r < 0)
        return 0 ;
    return iF[r] * F[n] % modg * iF[n - r] % modg ;
}
...
bool solve(){
    ...
    dp[2][1] = 0 ;
    for(int i = 4 ; i <= n ; i += 2){
        dp[i][0] = (dp[i - 2][1] + NCR(i - 1 , i / 2)) % modg ;
        dp[i][1] = (dp[i - 2][0] + NCR(i - 2 , i / 2)) % modg ;
    }
    cout << dp[n][0] << " " << dp[n][1] << " " << 1 << nl ;
    return true ;
}
