...
bool solve(){
    ...
    while(m--){
        ...
        cin >> index >> x ;
        index -= 1 ;
        if(index != n - 1 and v[index] == v[index + 1]) sum += (n - (index + 1)) * (index + 1) ;
        if(index != 0 and v[index] == v[index - 1]) sum += (n - index) * index ;
        if(index != n - 1 and x == v[index + 1]) sum -= (n - (index + 1)) * (index + 1) ;
        if(index != 0 and x == v[index - 1]) sum -= (n - index) * index ;
        v[index] = x ;
        cout << sum << nl ;
    }
    return true ;
