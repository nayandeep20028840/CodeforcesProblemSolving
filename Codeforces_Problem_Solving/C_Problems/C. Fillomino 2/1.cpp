...
bool recursion(vector<vector<int > > &v, 
            vector<vector<bool > > &visited , int i , int j , int num , int c){
    if(c == num - 1){
        return true ;
    }
        ...
        v[i][j - 1] = num ;
        visited[i][j - 1] = true ;
        recursion(v , visited , i , j - 1 , num , ++c) ;
    }
    else if(i < v.size() - 1 && visited[i + 1][j] == false){
        v[i + 1][j] = num ;
        visited[i + 1][j] = true ;
        recursion(v , visited , i + 1 , j , num , ++c) ;
    }else{
        return false ;
    }
    return true ;
...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        int c = 0 ;
        if(recursion(v , visited , i , i , v[i][i] , c)){
