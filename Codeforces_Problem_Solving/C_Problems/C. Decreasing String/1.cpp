...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        int t = 0 ;
        while(!stack.empty() and stack.top() > v[i]){
            if(want >= k){
            ...
            // cout << i - t - 1 << " " << stack.top() << nl ;
            // ms.erase(ms.find({i - t - 1 , stack.top()})) ;
            int cnt = ms.order_of_key({i , 0}) ;
            // cout << cnt << nl ;
            ms.erase(ms.find_by_order(cnt - 1)) ;
            stack.pop() ;
            t += 1 ;
        }
        stack.push(v[i]) ;
