...
bool solve(){
    ...
    vector<customer > v(n) ;
    for(int i = 0 ; i < n ; ++i){
        cin >> v[i].time >> v[i].low >> v[i].high ;
    }
    int prev_low = m ;
    int prev_high = m ;
    int cur_time = 0 ;
    sort(all(v) , [&](customer a , customer b){
        return a.time < b.time ;
    }) ;
    int f = 0 ;
    for(int i = 0 ; i < n ; ++i){
        int have = v[i].time - cur_time ;
        cur_time = v[i].time ;
        int mn = min(v[i].low , v[i].high) ;
        int mx = max(v[i].low , v[i].high) ;
        int ek = prev_low ;
        int doo = prev_high ;
        if(v[i].low >= prev_low and v[i].low <= prev_high){
            prev_low = v[i].low ;
        }
        else{
            if(abs(mn - prev_low) < abs(mn - prev_high)){
                if(have >= abs(mn - prev_low)){
                    prev_low = mn ;
                }
                else{
                    if(v[i].low >= prev_low){
                ...
                if(have >= abs(mn - prev_high)){
                    prev_low = mn ;
                }
                else{
                    if(v[i].low >= prev_high){
                        prev_low = prev_high + have ;
                    }
                    else{
                        prev_low = prev_high - have ;
                    }
        ...
        if(v[i].high >= prev_low and v[i].high <= prev_high){
            prev_high = v[i].high ;
        }
        else{
            if(abs(mx - prev_high) > abs(mx - ek)){
                if(have >= abs(mx - ek)){
                    prev_high = mx ;
                }
                else{
        ...
        // cout << prev_low << " " << prev_high << " " << have << nl ;
        if(prev_high < mn || prev_low > mx){
            f = 1 ;
        }
    }
    if(f) no
