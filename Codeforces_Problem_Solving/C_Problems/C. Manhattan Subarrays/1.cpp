...
our problem reduces to (let indexed i,j,k) abs(bk-bi)=abs(bi-bj)+abs(bj-bk)
so we have to find next to next greater element
...
bool solve(){
    ...
    cin>>n;
    iv(v,n)
    vector<int>inc(n,1e6);
    stack<int>stack;
    for(int i=0;i<n;++i){
        while(!stack.empty() and v[stack.top()]<=v[i]){
            inc[stack.top()] = min(i,inc[stack.top()]);
            stack.pop();
        }
        stack.push(i);
    }
    vector<int>dec(n,1e6);
    while(!stack.empty()) stack.pop();
    for(int i=n-1;i>=0;--i){
        while(!stack.empty() and v[stack.top()]>=v[i]){
            dec[stack.top()] = min(i,dec[stack.top()]);
            stack.pop();
        }
        stack.push(i);
    }
    check1(inc)
    check1(dec)
    vector<int>want1(n,1e6);
    for(int i=0;i<n;++i){
        if(inc[i]!=1e6 and dec[i]!=1e6){
            want1[dec[i]]=min(inc[i],want1[dec[i]]);
        }
    }
    inc.assign(n,1e6);
    dec.assign(n,1e6);
    ...
    check1(inc)
    check1(dec)
    vector<int>want2(n,1e6);
    for(int i=0;i<n;++i){
        if(inc[i]!=1e6 and dec[i]!=1e6){
            want2[dec[i]]=min(inc[i],want2[dec[i]]);
        }
    }
    cout<<nl;
    check1(want1)
    check1(want2)
    vector<int>need(n,1e6);
    for(int i=0;i<n;++i){
        need[i]=min(want1[i],want2[i]);
    }
    check1(need)
    int ans=n*(n+1)/2;
    int prev=0;
