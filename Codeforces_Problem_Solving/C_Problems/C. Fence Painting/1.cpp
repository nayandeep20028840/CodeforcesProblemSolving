...
bool solve(){
    ...
    map<int , int > mp1 ;
    vector<int> mp2[n + m + 10] ;
    for(int i = 0 ; i < n ; ++i){
        if(a[i] != b[i]){
            mp1[b[i]] ++ ;
            if(mp2[b[i]].size() < mp3[b[i]])
            mp2[b[i]].pb(i + 1) ;
        }
        if(b[i] == c[m - 1]){
            mp2[b[i]].pb(i + 1) ;
    ...
    if(f) no
        ...
        yes
        vector < int > cnt(n + m + 10 , 0) ;
        // cout << cn[1] << nl;
        for(int i = 0 ; i < m ; ++i){
            if(mp2[c[i]].size() && cnt[c[i]] < mp2[c[i]].size())
            cout << mp2[c[i]][cnt[c[i]]++] << " " ;
            else if(mp2[c[i]].size() && cnt[c[i]] >= mp2[c[i]].size())
            cout << mp2[c[i]][mp2[c[i]].size() - 1] << " " ;
            else
            cout << mp2[c[m - 1]][mp2[c[m - 1]].size() - 1] << " " ;
            // for(auto &j : mp2[c[i]]){
            //     cout << j << " " ;
            // }
        }
