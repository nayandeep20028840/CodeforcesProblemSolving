...
bool solve(){
    ...
    int prev = n ;
    if(n == 1) cout << 2 << nl ;
    // else if(n == 2) cout << 5 << nl ;
    // else if(n == 3) cout << 7 << nl ;
    // else if(n == 4) cout << 10 << nl ;
    else{
        for(int i = 1 ; i < v.size() ; ++i){
            lcm = LCM(lcm , v[i]) ;
            left = n / lcm ;
            ans = mod_ADDITION(ans , mod_MULTIPLICATION
                (mod_SUBTRACTION(prev , left , mod97) , v[i] , mod97) , mod97) ;
            // cout << left << " " << ans << " " << lcm << nl ;
            if(left == 0) {
                // ans = (ans + v[i + 1]) % mod97 ;
                break ;
            }
