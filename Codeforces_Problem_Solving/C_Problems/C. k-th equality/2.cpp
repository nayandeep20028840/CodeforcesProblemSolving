...
bool solve(){
    int a , b , c , k ;
    cin >> a >> b >> c >> k ;
    int st_a = power(10 , a - 1) ;
    int en_a = power(10 , a) - 1 ;
    int st_b = power(10 , b - 1) ;
    int en_b = power(10 , b) - 1 ;
    int st_c = power(10 , c - 1) ;
    int en_c = power(10 , c) - 1 ;
    if(st_a + st_b > en_c || c < a || c < b || (en_a + en_b) < st_a){
        cout << -1 << nl ;
        return true ;
    }
    int t = 0 ;
    for(int i = st_a ; i <= en_a ; ++i){
        if(i + st_b >= st_c && i + st_b <= en_c){
            st_a = i ;
            t = 1 ;
            break ;
        }
        if(i + en_b >= st_c && i + en_b <= en_c){
            st_a = i ;
            st_b = st_c - i ;
            t = 1 ;
            break ;
