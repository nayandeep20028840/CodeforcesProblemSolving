...
void suffixsum(vector<int > v , int n){
    cout << n - 1 << nl ;
    for(int i = n - 1 ; i > 0 ; --i){
        cout << i << " " << i + 1 << nl ;
    }
...
bool solve(){
    ...
    if(count_neg <= 7 || count_pos <= 7){
        if(count_pos > count_neg){  
            int num = mx ;
            while(num < 21 && num){
                num *= 2 ;
                ans.pb({max_ind + 1 , max_ind + 1}) ;
            }
            for(int i = 0 ; i < n ; ++i){
                if(v[i] < 0){
                    ans.pb({i + 1 , max_ind + 1}) ;
                }
            }
            for(int i = 0 ; i < n - 1 ; ++i){
                ans.pb({i + 2 , i + 1}) ;
            ...
            while(num > -20 && num){
                num *= 2 ;
                ans.pb({min_ind + 1 , min_ind + 1}) ;
            }
            // cout << num << " " << mx << " " << ans.size() << " " ;
            for(int i = 0 ; i < n ; ++i){
                if(v[i] > 0){
                    ans.pb({i + 1 , min_ind + 1}) ;
                }
            }
            for(int i = n - 1 ; i > 0 ; --i){
                ans.pb({i , i + 1}) ;
