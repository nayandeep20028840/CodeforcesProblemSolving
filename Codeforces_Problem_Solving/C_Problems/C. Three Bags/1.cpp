...
bool solve(){
    ...
    int sum3 = accumulate(all(c) , 0ll) ;
    sort(all(a)) ;
    sort(all(b)) ;
    sort(all(c)) ;
    int ans = INT_MIN ;
    // cout << sum1 << " " << sum2 << " " << sum3 << nl ;
    if(a.size() && b.size()) ans = max(ans , sum1 + sum2 + sum3 - 2 * a[0] - 2 * b[0]) ;
    if(a.size() && c.size()) ans = max(ans , sum1 + sum2 + sum3 - 2 * a[0] - 2 * c[0]) ;
    if(b.size() && c.size()) ans = max(ans , sum1 + sum2 + sum3 - 2 * c[0] - 2 * b[0]) ;
