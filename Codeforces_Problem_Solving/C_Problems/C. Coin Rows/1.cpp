...
bool solve(){
    ...
    vector<vector<int>> sf_pf(2 , vector<int> (n));
    sf_pf[0][n - 1] = v[0][n - 1];
    sf_pf[1][0] = v[1][0];
    for(int i = 0 ; i < 2 ; ++i){
        if(i % 2 == 0){
            for(int i = n - 2 ; i > 0 ; --i){
                sf_pf[0][i] = sf_pf[0][i + 1] + v[0][i];
            }
        }else{
            for(int i = 1 ; i < n ; ++i){
                sf_pf[1][i] = sf_pf[1][i - 1] + v[1][i];
            }
        }
    }
    int ans = v[0][0] + v[1][n - 1];
    for(int i = 0 ; i < 1 ; ++i){
        for(int j = 1 ; j < n ; ++j){
            // ans += max(v[i + 1][j - 1] , v[i][j]);
            if(sf_pf[i + 1][j - 1] > sf_pf[i][j]){
                if(j - 2 >= 0){
                    cout << max(sf_pf[i + 1][j - 2] , sf_pf[i][j]) << endl;
                    return true;
                }else{
                    cout << sf_pf[i][j] << endl;
                    return true;
