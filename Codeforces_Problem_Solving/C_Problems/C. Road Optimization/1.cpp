...
int n , l , k , dp[501][501];
vector<int> dist ;
vector<int> speed ;

int func(int index , int k_till_now){
    if(index == n) return 0 ;
    if(dp[index][k_till_now] != -1) return dp[index][k_till_now] ;
    int ans = 999999999999 ;
    for(int i = index ; i < n ; ++i){
        if(k_till_now + i - index > k) break;
        int dis = dist[i + 1] - dist[index] ;
        ans = min(ans , (dis*speed[index]) + func(i + 1 , k_till_now + i - index)) ;
    }
    return dp[index][k_till_now] = ans ;
}
bool solve(){
