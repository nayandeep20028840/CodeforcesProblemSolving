...
bool solve(){
    ...
    for(int i = n - 1 ; i > 0 ; --i){
        ...
        if(b[i] == a[i]){
            ...
            ans = max(ans ,  2 + dp[i + 1] + abs(b[i] - a[i])) ; // very very necessary max answer can be found in between
            // dp[i] = max(dp[i] , 2 + dp[i + 1] + abs(b[i] - a[i])) ;
            dp[i] = max(dp[i] , 2 + dp[i + 1] + abs(b[i] - c[i - 1]) + abs(a[i] - 1)) ;
        }
        if(i == 1){
            // ans = max(ans ,  2 + dp[i + 1] + abs(b[i] - a[i])) ; // not necessary i don't know why
            // i think because dp[i] that is dp[1] = -1 
            dp[i] = max(dp[i] , 2 + dp[i + 1] + abs(b[i] - a[i])) ;
        }
        // cout << dp[i] << " " ;
    }
