...
Imagine an apple orchard with a tree that has apples numbered 1, 2, 3, and so on. The owner of the apple tree has two numbers, l
and r, which represent a range of apple numbers. The owner wants to count how many apples in that range are badapples
.

In this case, a bad apples is an apple whose number has exactly two divisors.

Your task is to find out the total count of bad appleswithin the given range [l, r].

[] – denotes both inclusive and 1 is not a bad
apple
.
Input
The first line contains a single integer t (1≤t≤10) — the number of test cases.

For each test case, The Single line contains two integers l and r(1≤l≤r≤109and r−l≤ 105) — denoting the range of apple numbers.

Output
The output contains t lines each containing an answer for the test case – total count of bad appleswithin the given range [l, r].
...
const int LOG = 21 ;
int SPF[N + 1] ;
vector<bool > isPrime(N + 1 , 1) ; // vector<bool> is between 1.4x and 1.7x faster than using a vector<char>.
...
vector<int> givemeprimes(int n) {int*arr = new int[n + 1](); vector<int> vect; for(int i = 2; i <= n; i++)if (arr[i] == 0) {vect.push_back(i); for(int j = 2 * i; j <= n; j += i)arr[j] = 1;} return vect;}

bool solve(){
    int l , r ;
    cin >> l >> r ;
    vector<int > primes = givemeprimes(sqrtl(r)) ;
    // check1(primes)
    vector<int > ans(r - l + 1 , 1) ;
    int len = primes.size() ;
    if(l <= 1) ans[0] = 0 ;
    for(auto &pr : primes){
        int firstmultiple = ((l + pr - 1) / pr) * pr ;
