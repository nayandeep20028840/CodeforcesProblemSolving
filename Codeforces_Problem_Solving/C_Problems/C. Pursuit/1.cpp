...
bool solve(){
    ...
    int sum2 = 0 ;
    queue<int> a2;
    for(int i = n - start ; i < n ; ++i) {sum1 += a[i]; a2.push(a[i]);}
    for(int i = n - start ; i < n ; ++i) {sum2 += b[i]; }
    stack<int> b3;
    for(int i = 0 ; i < n / 4 ; ++i) {b3.push(b[i]);}
    if(sum1 >= sum2){cout << 0 << endl; return true;}
    int ans = 0;
    int count = 0;
    for(int i = 0 ; i < n ; ++i){
        sum1 += 100;
        ans++;
        if(sum1 >= sum2){cout << ans << endl; return true;}
        count++;
        if(count % 4 == 0){
            sum1 -= a2.front(); a2.pop();
            if(b3.empty())
            sum2 += b3.top(); b3.pop();
        }
    }
