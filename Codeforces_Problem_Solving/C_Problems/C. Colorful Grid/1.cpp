...
bool solve(){
    ...
    cin >> n >> m >> k ;
    int tot = n + m - 2 ;
    if(k < tot || tot % 2 != k % 2) no
    else{
        yes
        int rem = k - tot ;
        // cout << rem << nl ;
        if(rem == 0){
            vector<vector<int > > first_matrix(n , vector<int > (m - 1 , 0)) ;
            int carry = 1 ;
            for(int i = 0 ; i < m - 1 ; ++i){
                first_matrix[0][i] = carry ;
                carry = 1 - carry ; 
            }
            vector<vector<int > > second_first_matrixrix(n - 1 , vector<int > (m , 0)) ;
            for(int i = 0 ; i < n - 1 ; ++i){
                second_first_matrixrix[i][m - 1] = carry ;
                carry = 1 - carry ;
            }
            for(int i = 0 ; i < n ; ++i){
                for(int j = 0 ; j < m - 1 ; ++j){
            ...
            for(int i = 0 ; i < n - 1 ; ++i){
                for(int j = 0 ; j < m ; ++j){
                    if(second_first_matrixrix[i][j] == 1){
                        cout << "R" << " " ;
                    }
                    else{
                        cout << "B" << " " ;
            ...
            second_first_matrixrix[n - 2][m - 1] = carry ;
            carry = 1 - carry ;
            first_matrix[n - 1][m - 2] = carry ;
            carry = 1 - carry ;
            second_first_matrixrix[n - 2][m - 2] = carry ;
            carry = 1 - carry ;
            first_matrix[n - 2][m - 2] = carry ;
            carry = 1 - carry ;
            for(int i = 0 ; i < n ; ++i){
                for(int j = 0 ; j < m - 1 ; ++j){
                    if(first_matrix[i][j] == 1){
