...
bool solve(){
    ...
    if(s[0] == '?') s[0] = '(' ;
    if(s[n - 1] == '?') s[n - 1] = ')' ;
    int OPEN_count = count(all(s) , '(') ;
    int CLOSE_count = count(all(s) , ')') ;
    int MARK_count = count(all(s) , '?') ;
    int t = 0 ;
    int f = 0 ;
    string check = s ;
    int copy1 = OPEN_count ;
    int copy2 = CLOSE_count ;
    for(int i = 0 ; i < n ; ++i){
        if(s[i] == '?'){
            if(copy1 < n / 2){
                check[i] = '(' ;
    ...
    // cout << OPEN_count << " " << CLOSE_count << " " << MARK_count << nl ;
    if(CLOSE_count == n / 2) f = 1 ;
    if(OPEN_count == n / 2) f = 1 ;
    int sum2 = -1 ;
    if(!f){
        sum2 = 0 ;
        for(int i = 0 ; i < n ; ++i){
            if(s[i] == '?'){
                if(OPEN_count == n / 2 - 1 && t == 0){
                    CLOSE_count += 1 ;
                    s[i] = ')' ;
                    t = 1 ;
                }else{
