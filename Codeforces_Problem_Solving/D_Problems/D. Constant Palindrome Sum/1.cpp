...
template<class T>using Tree = tree<T, null_type, less_equal<T>, rb_tree_tag, tree_order_statistics_node_update> ; // here T can be anything like pair,vector,map,pair_of_pair // USE it like Tree<pair<int,int>> s; for pairs
...
bool solve(){
    ...
    */
    ...
    Tree<int > MIN_S ;
    for(int i = 0 ; i < n / 2 ; ++i){
        MAX_S.insert(max(v[i] , v[n - i - 1])) ;
        MIN_S.insert(min(v[i] , v[n - i - 1])) ;
        mp[v[i] + v[n - i - 1]] += 1 ;
    }
    // for(auto &i : mp){
    //     cout << i.F << " " << i.S << nl ;
    ...
    for(int i = 1 ; i <= max(n , k + k) ; ++i){
        int c1 = n / 2 - MIN_S.order_of_key(i) ;
        int c2 = MAX_S.order_of_key(i - k) ;
        int c3 = n / 2 - (c1 + c2) - mp[i] ;
        // cout << c1 << " " << c2 << " " << c3 << nl ;
        c1 *= 2 ;
        c2 *= 2 ;
        ans = min(ans , max(0ll , c1 + c2 + c3)) ;
    }
    cout << ans << nl ;
    return true ;
}
