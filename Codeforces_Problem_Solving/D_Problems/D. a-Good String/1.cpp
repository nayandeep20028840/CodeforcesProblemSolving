...
bool solve(){
    ...
    string s ;
    cin >> s ;
    function<int(int, int, char)> divide_and_conquer = [&](int l , int r , char c) -> int {
        if(l == r){
            if(s[l] == c) return 0 ;
            else return 1 ;
        }
        ...
        int ans = min(first + divide_and_conquer(mid + 1 , r , c + 1) , 
                        second + divide_and_conquer(l , mid , c + 1)) ;

        return ans ;
    };

    cout << divide_and_conquer(0 , n - 1 , 'a') << nl ;
