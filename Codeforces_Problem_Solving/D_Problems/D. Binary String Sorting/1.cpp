...
bool solve(){
    ...
    int mn = LLONG_MAX ;
    for(int i = 0 ; i < n + 1 ; ++i){
        mn = min(mn , ans + ((int)((pf[i] + sf[i]) * (1e12 + 1)))) ;
    }
    // cout << s << " " << mn << nl ;
    // check1(pf)
    // check1(sf)
    // mn += ans ;
    vector<int > pf2(n + 1 , 0) , sf2(n + 1 , 0) ;
    for(int i = 0 ; i < n ; ++i){
        if(ss[i] == '1'){
            pf2[i + 1] = pf2[i] + 1 ;
        }else{
    ...
    int mn2 = LLONG_MAX ;
    for(int i = 0 ; i < n + 1 ; ++i){
        mn2 = min(mn2 , ((int)((pf2[i] + sf2[i]) * (1e12 + 1)))) ;
    }
    // cout << ss << " " << mn2 << nl ;
    // check1(pf2)
    // check1(sf2)
    ...
    vector<int > pf3(n + 1 , 0) , sf3(n + 1 , 0) ;
    for(int i = 0 ; i < n ; ++i){
        if(s[i] == '1'){
            pf3[i + 1] = pf3[i] + 1 ;
        }else{
    ...
    int mn3 = LLONG_MAX ;
    for(int i = 0 ; i < n + 1 ; ++i){
        mn3 = min(mn3 , ans + ((int)((pf3[i] + sf3[i]) * (1e12 + 1)))) ;
    }
    cout << min({mn3 , mn , mn2}) << nl ;
    return true;
}
