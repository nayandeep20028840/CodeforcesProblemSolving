...
bool solve(){
    ...
    function<int(int , int) > func = [&](int i , int j) -> int {
        ...
        int third = INF ;
        if(v[i] + 1 == v[i + 1]){
            first = min(x , (i + 1 == v.size() - 1 ? 2 * y : y)) + func(i + 2 , j) ;
        }
        else{
            first = min(x * (v[i + 1] - v[i]) , y) + func(i + 1 , j) ;
        }
        if(v[i] + 1 == v[j]){
            second = min(x , (i + 1 >= j - 1 ? 2 * y : y)) + func(i + 1 , j - 1) ;
        }
        else{
            second = min(x * (v[j] - v[i]) , y) + func(i + 1 , j - 1) ;
        }
        if(v[j] - 1 == v[j - 1]){
            third = min(x , (j - 1 == 0 ? 2 * y : y)) + func(i , j - 2) ;
        }
        else{
            third = min(x * (v[j] - v[j - 1]) , y) + func(i , j - 2) ;
        }
        return dp[i][j] = min({first , second , third}) ;
    };
    cout << func(0 , v.size() - 1) << nl ;
    return true ;
}
