...
bool solve(){
    ...
    ans = max(ans , mx - mn) ;
    // for(int i = 0 ; i < n ; ++i){
        // if(l[i] > maxr || r[i] < maxl) {
        //     ans = maxr - maxl + 1 ;
        //     break ; 
        // }
    //     ans = max(ans , maxr - maxl + 1 - (l[i] - maxr + 1)) ;
    //     ans = max(ans , maxr - maxl + 1 - (r[i] - maxl + 1)) ;
    // }
    for(int i = 0 ; i < n ; ++i){
        // cout << ans << " " ;
        int t = r[i] - l[i] + 1 ;
        if(l[i] > rr || r[i] < large) {
            ...
            ans = max(ans , t - (rr - l[i] + 1)) ;
            ans = max(ans , t - (r[i] - large + 1)) ;
        }
        // ans = max(ans , t - ())
    }
    // cout << mn << " " << mx << " " << maxr << " " << maxl << " " << rr << " " << large << nl ;
    cout << ans * 2 << nl ;
    return true;
}
