...
bool solve(){
    ...
    for(int i = 30 ; i >= 0 ; --i) {
        ...
        for(int j = 0 ; j < n ; j++) {
            ...
            if(bit) cnt += 1 ;
            if(cnt & 1){
                ans = (ans + (((((j + 1) * even_cnt) % MOD) - even_sum + MOD) % MOD) % MOD) % MOD ;
                odd_cnt += 1 ;
                odd_sum = (odd_sum + (j + 1)) % MOD ;
            }
            else{
                ans = (ans + (((((j + 1) * odd_cnt) % MOD) - odd_sum + MOD) % MOD) % MOD) % MOD ;
                even_cnt += 1 ;
                even_sum = (even_sum + (j + 1)) % MOD ;
            }
        }
        rans = (rans + (((1 << i) * ans) % MOD)) % MOD ;
    }
    cout << rans << nl ;
    return true ;
}
