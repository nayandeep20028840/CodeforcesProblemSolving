...
bool solve(){
    ...
    sort(all(b)) ;
    sort(all(c)) ;
    vector<vector<vector<int > > > dp(r+1, vector<vector<int > > (g+1, (vector<int > (bb+1, -1)))) ;
    function<int(int, int, int)> recursion = [&](int i, int j, int k) -> int {
        if((i < 0 && j < 0) || (j < 0 && k < 0) || (k < 0 && i < 0)){
            return 0 ;
        }
        // we have used i + 1, j + 1, k + 1 for storing because if i < 0 and j > 0 and k > 0
        if(dp[i + 1][j + 1][k + 1] != -1) return dp[i + 1][j + 1][k + 1] ;
        int first = 0 ;
        int second = 0 ;
        int third = 0 ;
        if(i >= 0 && j >= 0) {first = a[i] * b[j] + recursion(i - 1 , j - 1 , k) ;}
        if(j >= 0 && k >= 0) {second = b[j] * c[k] + recursion(i , j - 1 , k - 1) ;}
        if(k >= 0 && i >= 0) {third = c[k] * a[i] + recursion(i - 1 , j , k - 1) ;}

        return dp[i + 1][j + 1][k + 1] = max({first , second , third}) ;
	};
    int ans = recursion(r - 1 , g - 1 , bb - 1) ;
    cout << ans << nl ;
    // for(int i = 0 ; i < r ; ++i){
    //     for(int j = 0 ; j < g ; ++j){
    //         for(int k = 0 ; k < bb ; ++k){
