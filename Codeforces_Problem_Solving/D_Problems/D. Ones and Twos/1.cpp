...
bool solve(){
    ...
    while(q--){
        ...
        if(num == 1){
            ...
            cin >> ind >> val ;
            ind -= 1 ;
            if(a[ind] == 1 and val == 2){
                if(one.front() == ind){
                    one.erase(one.begin()) ;
                }
                else if(one.back() == ind){
                    one.pop_back() ;
                }
                else{
                    auto it = lower_bound(all(one) , ind) ;
                    one.erase(it) ;
                }
                // cout << one.size() << " " ;
                a[ind] = 2 ;
            ...
            else if(a[ind] == 2 and val == 1){
                if(ind < one.front()){
                    one.insert(one.begin() , ind) ;
                }
                else if(ind > one.back()){
                    one.push_back(ind) ;
                }
                else{
                    auto it = lower_bound(all(one) , ind) ;
