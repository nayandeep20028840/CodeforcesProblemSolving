...
bool solve(){
    ...
    while(q--){
        ...
        // int val=ft.at(l,r);
        // cout<<abs(val)/2<<nl;
        int pos_val=ft_pos.at(l,r);
        int neg_val=ft_neg.at(l,r);
        // cout<<pos_val<<" "<<neg_val<<nl;
        if(neg_val+pos_val==0) cout<<0<<nl;
        else if((r-l+1)%2==0) cout<<2<<nl;
        else cout<<1<<nl;

        // cout<<++pos_val/2 + ++neg_val/2<<nl;
    }
    return true ;
}
