...
int rec(int i,int j){
    ...
    if(b.size()-j > a.size()-i){
        ans= min(abs(a[i]-b[j])+rec(i+1,j+1),rec(i,j+1));
    }
    else{
        ans=abs(a[i]-b[j])+rec(i+1,j+1);
    }
    return dp[i][j]=ans;
}
...
bool solve(){
    ...
    // check1(a)
    // check1(b)
    // dp=vector<vector<vector<int>>>(cnt1+1,vector<vector<int>>(cnt2+1,vector<int>(cnt2-cnt1+1,-1)));
