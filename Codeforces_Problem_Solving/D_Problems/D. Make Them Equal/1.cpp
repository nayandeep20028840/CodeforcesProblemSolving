...
int n ;
vector<int > dp1(1001 , INT_MAX);
int dp2[N];
vector<int > b ;
vector<int > c ;

int func(int index , int cost){
    if(dp2[cost] != -1) return dp2[cost] ;
    if(index >= n) return 0 ;
    if(dp1[b[index]] > cost){
        return dp2[cost] = func(index + 1 , cost) ;
    }else{
        return dp2[cost] = max(func(index + 1 , cost) , c[index] + func(index + 1 , cost - dp1[b[index]])) ;
    }
}
