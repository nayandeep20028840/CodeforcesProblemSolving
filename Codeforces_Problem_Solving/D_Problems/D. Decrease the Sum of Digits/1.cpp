...
bool solve(){
    ...
    for(int i = 0 ; i < str.size() ; ++i){
        if(!equal) sum += (str[i] - '0') ;
        else if((str[i] - '0') > 0) cal = 1 ;
        if(sum == s and !equal){
            equal = 1 ;
            ind = i ;
        }
        else if(sum > s and !equal){
            ind = i ;
            break ;
        }
    }
    // cout << sum << " " << equal << " " << ind << nl ;
    string ans = "" ;
    if(sum <= s and ind == n - 1){
        cout << 0 << nl ;
    }
    else if(equal and cal){
        int len = str.size() ;
        int take = 0 ;
        for(int i = len - 1 ; i >= ind ; --i){
            ...
            else if(str[i] != '0' and !take){
                take = 1 ;
                ans += char(9 - (str[i] - '0') + 1 + '0') ;
            }
            else if(take){
                ans += char(9 - (str[i] - '0') + '0') ;
            }
        }
