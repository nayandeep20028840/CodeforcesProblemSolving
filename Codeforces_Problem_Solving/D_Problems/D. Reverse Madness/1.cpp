...
bool solve(){
    ...
    sort(all(x)) ;
    // check1(x)  // working
    auto it = lower_bound(all(l) , x[0]) - l.begin() ;
    if(it == k) it -= 1 ;
    else if(l[it] != x[0]) it -= 1 ;
    int a = min(x[0] , r[it] + l[it] - x[0]) ;
    int b = max(x[0] , r[it] + l[it] - x[0]) ;
    vector<pair<int , int > > p ;
    p.pb({a , b}) ;
    vector<pair<pair<int , int > , pair<int , int > > > adj[k] ;
    adj[it].pb({{a , b} , {r[it] - l[it] , l[it]}}) ;
    for(int i = 1 ; i < q ; ++i){
        // if(x[i] == x[i - 1]) continue ;
        it = lower_bound(all(l) , x[i]) - l.begin() ;
        if(it == k) it -= 1 ;
        else if(l[it] != x[i]) it -= 1 ;
        a = min(x[i] , r[it] + l[it] - x[i]) ;
        b = max(x[i] , r[it] + l[it] - x[i]) ;
        adj[it].pb({{a , b} , {r[it] - l[it] , l[it]}}) ;
        p.pb({a , b}) ;
    }
    for(int i = 0 ; i < k ; ++i){
        ...
        // cout << adj[i][0].S.S << nl ;
        vector<int > pos ;
        for(int j = adj[i][0].S.S ; j <= adj[i][0].S.S + len ; ++j){
            pos.pb(j) ;
        }
        // check1(pos)
        for(int j = 0 ; j < pos.size() / 2 ; ++j){
            ...
            // cout << cnt << nl ;
            if(cnt % 2){
                swap(s[pos[j] - 1] , s[pos[pos.size() - 1 - j] - 1]) ;
            }
        }
    }
    cout << s << nl ;
