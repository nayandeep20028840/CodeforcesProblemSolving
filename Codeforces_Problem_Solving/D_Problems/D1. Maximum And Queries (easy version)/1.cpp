...
unsigned long long power(unsigned long long x, unsigned long long y){unsigned long long res = 1 ; while(y > 0) {if(y & 1) res = (res * (unsigned long long)x) ; x = ((unsigned long long)x * (unsigned long long)x) ; y = y >> 1 ;} return res ;}
        ...
        for(int j = 61 ; j >= 0 ; --j){
            ...
            for(int i = 0 ; i < n ; ++i){
                ...
                if(num & power(2 , j)){
                    ...
                    int sub = (num & (power(2 , j) - 1)) ;
                    // cout << sub << " " ;
                    num -= sub ;
                    tot += power(2 , j) ;
                    if(tot > 1e18){ // very important remember while using binary search
                        f = 1 ;
                        break ;
                    }
                    tot -= sub ;
