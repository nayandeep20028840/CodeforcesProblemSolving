...
bool solve(){
    ...
    int pre = 0 ;
    for(int i = 0 ; i < n ; ++i){
        p.pb({a[i] + b[i] , {a[i] , b[i]}}) ;
        pre += abs(a[i] - b[i]) ;
    }
    sort(all(p)) ;
    int ans = 0 ;
    for(int i = 0 ; i < n ; ++i){
        // cout << p[i].F << " " << p[i].S.F << " " << p[i].S.S << nl ;
    }
    ans += abs(p[0].S.F - p[n - 1].S.S) ;
    ans += abs(p[0].S.S - p[n - 1].S.F) ;
    // cout << ans << " " ;
    for(int i = 1 ; i < n - 1 ; ++i){
        ans += abs(p[i].S.F - p[i].S.S) ;
    }
    cout << max(ans , pre) << nl ;
    return true ;
}
