...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        if(s[i] == 'R') m[v[i]]++;
        else if(s[i] == 'B') m2[v[i]]++;

        if((v[i] > n && s[i] == 'R') || (v[i] < 1 && s[i] == 'B')){no return true;}
    }
    for(int i = 2 ; i <= n ; ++i){
        if(m2[1] > 1){no return true;}
        if(m2[i] > (i - m2[i - 1])){no return true;}
        m2[i] += m2[i - 1];
    }
    for(int i = n - 1 ; i > 0 ; --i){
        if(m[n] > 1){no return true;}
        if(m[i] > (n - i + 1 - m[i + 1])){no return true;}
        m[i] += m[i + 1];
    }
    yes
    return true;
