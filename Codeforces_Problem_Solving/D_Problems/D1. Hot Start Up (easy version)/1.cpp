...
bool solve(){
    ...
    function<int(int, int, int)> func = [&](int i, int prev1 , int prev2) -> int {
        if(i == n) return 0;
        if(dp[i][a[i]] != -1) return dp[i][a[i]] ;
        if(prev1 == a[i]){
            dp[i][a[i]] = hot[a[i]] + func(i + 1 , prev1 , prev2) ;
        }
        else if(prev2 == a[i]){
            dp[i][a[i]] = hot[a[i]] + func(i + 1 , prev1 , prev2) ;
        }else{
            dp[i][a[i]] = min(cold[a[i]] + func(i + 1 , prev1 , a[i]) , cold[a[i]] + func(i + 1 , a[i] , prev2)) ;
        }
        return dp[i][a[i]] ;
    };
    cout << func(0 , -1 , -1) << nl ;
