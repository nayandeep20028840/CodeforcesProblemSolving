...
bool solve(){
    ...
    for(int i = 1 ; i <= n ; ++i){
        if(mp1[i] == 0) count1 += 2 ;
        if(mp1[i] == 1 && t1 >= 2) count1 += 1 ;
        else t1 ++ ;
        if(mp2[i] == 0) count2 += 2 ;
        if(mp2[i] == 1 && t2 >= 2) count2 += 1 ;
        else t2 ++ ;
        // ans += min(mp1[i] , mp2[i]) / 2 ;
    }
    int cnt = 0 ;
    vector<pair<int , int > > p ;
    priority_queue<pair<int , int >> pq ;
    for(int i = 0 ; i < n ; ++i){
        if(mp1[i + 1] == 0 && mp2[i + 1] == 0){
            cnt += 2 ;
            pq.push({i + 1 , 2}) ;
            mp1[i + 1] ++ ;
            mp2[i + 1] ++ ;
        }
        else if(mp1[i + 1] == 1 && mp2[i + 1] == 1 || mp1[i + 1] == 1 
        && mp2[i + 1] == 0 || mp1[i + 1] == 0 && mp2[i + 1] == 1) {
            cnt += 1 ;
            pq.push({i + 1 , 1}) ;
            mp1[i + 1] ++ ;
            mp2[i + 1] ++ ;
    ...
    while(pq.size() > 1){
        ...
        tt.S -= 1 ;
        t.S -= 1 ;
        if(tt.S > 0) p.pb({tt.F , tt.S}) ;
        if(t.S > 0) p.pb({t.F , t.S}) ;
    }
    check2(p)
    return true;
}
