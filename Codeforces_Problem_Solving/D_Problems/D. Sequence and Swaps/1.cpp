...
bool solve(){
    ...
    function<int(int,int,int)> func=[&](int i,int prev_max,int xx)->int{
        if(i >= n) return 0 ;
        if(prev_max > a[i]) return inf ;
        if(dp[i][xx] != -1) return dp[i][xx] ;
        int ans = inf ;
        if(a[i] > xx and prev_max <= xx){
            ans = min({ans , 1 + func(i + 1 , max(prev_max , xx) ,  a[i]) , func(i + 1 , max(prev_max , a[i]) , xx)}) ;
        }
        else{
            ans = min({ans , func(i + 1 , max(prev_max , a[i]) , xx)}) ;
        }
        return dp[i][xx] = ans ;
    };
    int t = func(0 , -1 , x) ;
