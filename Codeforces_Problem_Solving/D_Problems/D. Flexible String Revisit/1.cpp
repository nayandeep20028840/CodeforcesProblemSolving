...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        ...
        // cout << more.size() << " " << less.size() << nl ;
        while(more.size() && less.size()){
            for(int h = 0 ; h < m ; ++h){
                if(mat[more.back()][h] == 1 && mat[less.back()][h] == 0){
                    mat[more.back()][h] = 0 ;
                    mat[less.back()][h] = 1 ;
                    cnt[more.back()] -= 1 ;
                    cnt[less.back()] += 1 ;
                    // cout << cnt[more.back()] << " " << cnt[less.back()] << nl ;
                    if(cnt[less.back()] == div && cnt[more.back()] == div){
                        answer.emplace_back(more.back() , less.back() , h) ;
                        less.pop_back() ;
                        more.pop_back() ;
                        break ;
                    }
                    else if(cnt[less.back()] == div){
                        answer.emplace_back(more.back() , less.back() , h) ;
                        less.pop_back() ;
                        break ;
                    }
                    else if(cnt[more.back()] == div){
                        answer.emplace_back(more.back() , less.back() , h) ;
                        more.pop_back() ;
                        break ;
                    }
    ...
    cout << answer.size() << nl ;
    for(auto [xx , yy , zz] : answer){
        cout << xx + 1 << " " << yy + 1 << " " << zz + 1 << nl ;
    }
    return true ;
}
