...
bool solve(){
    ...
    iv(v , n)
    long double sum = v[0] ;
    vector<pair<long double , pair<long double , long double > > > ans ;
    for(long double i = 1 ; i < n ; ++i){
        ans.pb({(long double)(v[i]) - (long double)((i + 1) * c) , {v[i] , (i + 1) * c}}) ;
    }
    sort(all(ans), [&](pair<long double , pair<long double , long double > > &a 
    , pair<long double , pair<long double , long double > > &b){
        return (long double)a.F > (long double)b.F ;
    } );
    long double f = 0 ;
    for(long double i = 0 ; i < ans.size() ; ++i){
        sum += ans[i].S.F ;
        // cout << sum << " " ;
        if((long double)sum < (long double)ans[i].S.S){
            f = 1 ;
            break ;
        }
    }
