...
bool solve(){
    ...
    for(int i = 0 ; i < 60 ; ++i){
        powers_of_two.pb(1ll << i) ;
    }
    sort(all(v)) ;
    reverse(all(v)) ;
    // check1(v)
    // check1(powers_of_two)
    int sum = accumulate(all(v) , 0LL) ;
    // cout << sum << nl ;
    if(sum % n){no return true ;}
    sum /= n ;
    ...
    for(int i = 0 ; i < n ; ++i){
        int t = abs(sum - v[i]) ;
        if(sum < v[i]){
            auto it = upper_bound(all(powers_of_two) , t) ;
            // cout << *it << " " << sum - (v[i] - *it) << nl ;
            if(*it > v[i] || !(isPowerOfTwo(sum - (v[i] - *it)))){no return true ;}
            for(int j = 0 ; j < 31 ; ++j){
                if(*it & (1ll << j)){
                    bid[j] += 1 ;
                }
            ...
            auto it = upper_bound(all(powers_of_two) , t) ;
            // cout << *it << " " << (v[i] + *it) - sum << nl ;
            if(!(isPowerOfTwo((v[i] + *it) - sum))){no return true ;}
            for(int j = 0 ; j < 31 ; ++j){
                if(*it & (1ll << j)){
                    bid[j] -= 1 ;
                }
