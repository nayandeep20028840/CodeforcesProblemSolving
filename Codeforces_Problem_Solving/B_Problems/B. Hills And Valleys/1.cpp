        ...
        if(cnt[i] == 1){
            ...
            if(v[i] > v[i - 1] && v[i] > v[i + 1]){
                if(cnt[i - 1] == 1 && cnt[i + 1] == 1){
                    ...
                    // cout << i + 2 << " " ;
                    if(i + 2 < n){
                        if((v[i + 1] > t) && (v[i + 1] > v[i + 2])) mn = min(mn , total - 1) ;
                        else mn = min(mn , total - 2) ;
                    }else{
                        mn = min(mn , total - 2) ;
                    }
                }
                    ...
                    int tt = v[i + 1] ;
                    if(i - 2 >= 0){
                        if(v[i - 1] > tt && v[i - 1] > v[i - 2]) mn = min(mn , total) ;
                        else if(v[i - 1] < tt && v[i - 1] < v[i - 2]) mn = min(mn , total) ;
                        else mn = min(mn , total - 1) ;
                    }
                    if(i + 2 < n){
                        if(v[i + 1] > t && v[i + 1] > v[i + 2]) mn = min(mn , total) ;
                        else if(v[i + 1] < t && v[i + 1] < v[i + 2]) mn = min(mn , total) ;
                        else mn = min(mn , total - 1) ;
                    }
                    if(i - 2 < 0 || i + 2 >= n){
                        mn = min(mn , total - 1) ;
