...
bool solve(){
    ...
    cin >> s ;
    int n = s.size() ;
    vector<int > pf_zero(n , 0),pf_one(n , 0) , sf_zero(n , 0),sf_one(n , 0) ;
    pf_zero[0] = ((s[0] == '0') ? 0 : 1) ;
    pf_one[0] = ((s[0] == '1') ? 0 : 1) ;
    sf_zero[n - 1] = ((s[n - 1] == '0') ? 0 : 1) ;
    sf_one[n - 1] = ((s[n - 1] == '1') ? 0 : 1) ;
    for(int i = 1 ; i < n ; ++i){
        if(s[i] == '0'){
            pf_zero[i] = pf_zero[i - 1] ;
            pf_one[i] = pf_one[i - 1] + 1 ;
        }else{
            pf_zero[i] = pf_zero[i - 1] + 1 ;
            pf_one[i] = pf_one[i - 1] ;
        }
    }
    for(int i = n - 2 ; i >= 0 ; --i){
        if(s[i] == '0'){
            sf_zero[i] = sf_zero[i + 1] ;
            sf_one[i] = sf_one[i + 1] + 1 ;
        }else{
            sf_zero[i] = sf_zero[i + 1] + 1 ;
            sf_one[i] = sf_one[i + 1] ;
        }
    }
    // check1(pf_zero)
    ...
    int ans = INT_MAX ;
    for(int i = 0 ; i < n ; ++i){
        ans = min(ans , pf_zero[i] + sf_one[i]) ;
    }
    int ans2 = INT_MAX ;
    for(int i = 0 ; i < n ; ++i){
        ans2 = min(ans2 , pf_one[i] + sf_zero[i]) ;
    }
    cout << min(ans , ans2) - 1 << nl ;
    return true;
}
