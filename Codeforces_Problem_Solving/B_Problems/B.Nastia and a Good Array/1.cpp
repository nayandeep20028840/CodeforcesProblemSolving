...
// how to declare dp vector<vector<int>> dp(n + 1 , vector<int> (3 , -1))
vector<int>ans;
bool solve(){
    int n ;
    cin >> n ;
    ...
    for(int i = 0 ; i < n - 1 ; ++i){
        ...
        // }else{
            if(v[i] < v[i + 1]){
                auto it = ub(all(ans) , v[i + 1]);
                v[i + 1] = *it ;
                p.pb({{i + 1 , i + 1 + 1} , {v[i] , v[i + 1]}});
            }else{
                auto it = ub(all(ans) , v[i]);
                v[i] = *it ;
                // auto itt = ub(all(ans) , v[i]);
                // v[i + 1] = *itt ;
                p.pb({{i + 1 , i + 1 + 1} , {v[i] , v[i + 1]}});
            }
        // }
