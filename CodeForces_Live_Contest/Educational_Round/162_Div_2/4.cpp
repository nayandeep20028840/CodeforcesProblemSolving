...
bool solve(){
    ...
    vector<int > rans(n , inf) ;
    for(int i = 0 ; i < n ; ++i){
        int last = left.size() ;
        if(left.size()) last = upper_bound(all(left) , i) - left.begin() ;
        int sum_last = upper_bound(all(pf) , pf[i] + v[i]) - pf.begin() ;
        // cout << last << " " << sum_last << nl ;
        if(i < n - 1 and v[i + 1] > v[i]){
            ans[i] = 1 ;
        }
        else if(last == left.size() || sum_last == pf.size()){
            ans[i] = inf ;
        }
        else{
            ans[i] = max(sum_last - i , left[last] + 1 - i) ;
        }
    }
    for(int i = 0 ; i < n ; ++i){
        int last = right.size() ;
        if(right.size()) last = upper_bound(all(right) , i) - right.begin() ;
        int sum_last = upper_bound(all(sf) , sf[i] + vv[i]) - sf.begin() ;
        // cout << last << " " << sum_last << " " ;
        if(i < n - 1 and vv[i + 1] > vv[i]){
            // cout << "ft" << nl ;
            rans[n - 1 - i] = 1 ;
        }
        else if(last == right.size() || sum_last == sf.size()){
            // cout << "sc" << nl ;
            rans[n - 1 - i] = inf ;
        }
        else{
            // cout << "th" << nl ;
            rans[n - 1 - i] = max(sum_last - i , right[last] + 1 - i) ;
        }
    }
    // check1(ans)
    // check1(rans)
    for(int i = 0 ; i < n ; ++i){
        ans[i] = min(ans[i] , rans[i]) ;
    }
    for(int i = 0 ; i < n ; ++i){
