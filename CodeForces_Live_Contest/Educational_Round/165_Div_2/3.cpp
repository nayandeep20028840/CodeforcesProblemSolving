...
int rec(int i,int taken){
    ...
    int ans=0;
    if(i == 0){
        if(i < n - 1 and taken < k){
            ans += min(a[i + 1] + rec(i + 1 , taken + 1) , a[i] + rec(i + 1 , taken)) ;
        }
        else{
            ans += a[i] + rec(i + 1 , taken) ;
        }
    }
    else{
        if(i < n - 1){
            ...
            if(taken < k){
                ans += min({a[i] + rec(i + 1 , taken), a[i - 1] + rec(i - 1 , taken + 1)}) ;
            }
            else{
                ans += a[i] + rec(i + 1 , taken);
            }
