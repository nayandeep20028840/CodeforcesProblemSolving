...
bool solve(){
    ...
    while(q--){
        ...
        l -= 1 ;
        if(l == 0){
            long long it = lower_bound(all(pf) , u) - pf.begin() ;
            // cout << it << " " << pf[it] << " " ;
            if(it == n){
                cout << it << " " ;
            }
                ...
                if(pf[it] > u){
                    if((u - pf[it - 1]) >= pf[it] - u) cout << max(l + 1ll , it + 1ll) << " " ;
                    else cout << max(l + 1ll , it) << " " ;
                }
                else{
                    cout << max(l + 1ll , it + 1ll) << " " ;
                }
            }
        }
        else{
            long long it = lower_bound(pf.begin() , pf.end() , pf[l - 1] + u) - pf.begin() ;
            // cout << pf[l - 1] + u << " " ;
            // cout << it << " " << pf[it] << " " << pf[it - 1] << " " ;
            if(it == n){
                cout << it << " " ;
            }
            else if(pf[l - 1] + u < pf[it]){
                if((pf[l - 1] + u - pf[it - 1]) >= pf[it] - pf[l - 1] - u) cout << max(l + 1ll , it + 1ll) << " " ;
                else cout << max(l + 1ll , it) << " " ;
            }
            else{
                cout << max(l + 1ll , it + 1ll) << " " ;
