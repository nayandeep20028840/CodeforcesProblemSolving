...
bool solve(){
    ...
    vector<int>bodya(n,0);
    vector<int>sasha(n,0);
    vector<int>vis(n+1,0);
    function<void(int,int,int)> dfs=[&](int child,int parent,int j)->void{
        if(j>=k) return;
        // cout<<child<<nl;
        for(auto &i : adj[child]){
            ...
            vis[child]=1;
            if(j>0){
                bodya[j] = bodya[j-1] - ((k - j) * a[parent-1]);
            }
            bodya[j]+=(k-j)*a[child-1];
            dfs(i,child,j+1);
        }
    };
    dfs(PB,0,0);
    vis.assign(n+1,0);
    // check1(vis)
    function<void(int,int,int)> dfs1=[&](int child,int parent,int j)->void{
        if(j>=k) return;
        // cout << child << nl;
        for(auto &i : adj[child]){
            if(vis[child]) continue;
            vis[child]=1;
            if(j>0){
                sasha[j] = sasha[j-1] - ((k - j) * a[parent-1]);
            }
            sasha[j]+=(k-j)*a[child-1];
            dfs1(i,child,j+1);
        }
    };
    dfs1(PS,0,0);
    // check1(bodya)
    // check1(sasha)
    int mx1=*max_element(all(bodya));
    int mx2=*max_element(all(sasha));
    if(mx1>mx2) cout << "Bodya" << nl;
    else if(mx1<mx2) cout << "Sasha" << nl ;
    else cout<<"Draw" << nl ;
    return true ;
}
