...
bool solve(){
    ...
    // vector<vector<int>> vv(n , vector<int>(30 , 0)); 
    int vv[n][30]{};
    for(int i = 0 ; i < n ; ++i){
        for(int j = 0 ; j < 30 ; ++j){
            if((v[i] >> j) & 1 == 1){
    ...
    while(q--){
        ...
        if((y - x) == 0){
            ...
            for(int i = 0 ; i < 30 ; ++i){
                if(x > 0){
                    if((vv[y][i] - vv[x - 1][i]) % 2 == 1){
                        f = 1;
                    }else{
                        if((vv[y][i] - vv[x - 1][i]) == (y - x + 1)) danger = 1;
                        if((vv[y][i] - vv[x - 1][i]) == 0) zero++;
                    }
                }else{
                    if((vv[y][i]) % 2 == 1){
                        f = 1;
                    }else{
                        if(vv[y][i] == (y + 1)) danger = 1;
                        if(vv[y][i] == 0) zero++;
                    }
                }
            }
            if(zero == 30) cout << 0 << nl;
            else if(f) cout << -1 << nl;
            else{
                if((y - x + 1) % 2 == 0) {
                    int ff = 0 ;
