...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        pf[i + 1] = pf[i] ^ v[i];
        sum[i + 1] = sum[i] + v[i];
        mp[pf[i + 1]][(i + 1) % 2].pb(i + 1);
    }
    while(q--){
        int x , y ;
        cin >> x >> y ;
        if( (pf[y] ^ pf[x - 1]) != 0) cout << -1 << nl;
        else if((sum[y] - sum[x - 1]) == 0) cout << 0 << nl;
        else if((y - x + 1) <= 2) cout << -1 << nl;
        else if((y - x + 1) % 2 == 1) cout << 1 << nl;
        else if(v[x - 1] == 0 || v[y - 1] == 0) cout << 1 << "\n";
        else{
            auto it = lb(mp[pf[x - 1]][x % 2].begin() , mp[pf[x - 1]][x % 2].end() , x);
            if(it != mp[pf[x - 1]][x % 2].end() && *it < y){
                cout << 2 << nl;
            }else{
                cout << -1 << nl;
            }
