...
int nCr(int n, int r){long long p = 1, k = 1;if (n - r < r)r = n - r;if (r != 0) {while (r) {p *= n;k *= r;long long m = __gcd(p, k);p /= m;k /= m;n--;r--;}}else return p;}
...
bool solve(){
    ...
    cin >> n ;
    iv(v , n)
    if(n == 2){cout << 1 << nl ; return true;}
    if(n == 3){cout << 5 << nl ; return true;}
    int ans = 0 ;
    // ans += fact[n] * inv_fact[2] * inv_fact[n - 2] ; 
    // ans += fact[n] * inv_fact[3] * inv_fact[n - 3] ; 
    ans += ncr(n , 2) ;
    ans += ncr(n , 3) ;
    // cout << ans << " " << n << nl ;
    for(int i = 3 ; i < n ; ++i){
        ...
        int c = 0 ;
        for(int j = 1 ; j < i ; ++j){
            if(abs(v[j - 1] - v[j]) <= abs(v[j + 1] - v[j]) && left == true){
                continue ;
            }
            else if(abs(v[j - 1] - v[j]) <= abs(v[j + 1] - v[j]) && left == false){
                left = true ;
                c += 1 ;
            }
            if(abs(v[j - 1] - v[j]) > abs(v[j + 1] - v[j])){
                left = true ;
                c += 1 ;
            }
        }
        // cout << ans << " " ;
        ans += (ncr(n , i + 1) * c) % mod97  ;
    }
    cout << ans << nl ;
    return true;
}
