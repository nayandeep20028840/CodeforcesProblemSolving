...
bool solve(){
    ...
    map<pair<int,int>,vector<int>>mp1,mp2,mp3;
    map<vector<int>,int>mp4;
    for(int i=0;i<n-2;++i){
        mp4[{a[i],a[i+1],a[i+2]}]+=1;
        mp1[{a[i],a[i+1]}].pb(a[i+2]);
        mp2[{a[i],a[i+2]}].pb(a[i+1]);
        mp3[{a[i+1],a[i+2]}].pb(a[i]);
    }
    int ans=0;
    // cout<<ans<<nl;
    for(int i=0;i<n-2;++i){
        ans += mp1[{a[i],a[i+1]}].size() - mp4[{a[i],a[i+1],a[i+2]}];
        ans += mp2[{a[i],a[i+2]}].size() - mp4[{a[i],a[i+1],a[i+2]}];
        ans += mp3[{a[i+1],a[i+2]}].size() - mp4[{a[i],a[i+1],a[i+2]}];
    }
    cout<<ans/2<<nl;
    return true ;
}
