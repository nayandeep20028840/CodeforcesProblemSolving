...
int rec(int i,int tot, int sum){
    if(i >= m) return 0;
    if(dp[i][sum] != -1 and sp[i][sum]>=tot) return dp[i][sum] ;
    int ans = 0 ;
    if(tot >= pr[i].first){
        ans = max({ans,pr[i].S+ rec(i+1,tot+x-pr[i].F,sum+pr[i].S),rec(i+1,tot+x,sum)});
    }
    else{
        ans=max(ans,rec(i+1,tot+x,sum));
    }
    sp[i][sum]=tot;
    return dp[i][sum]=ans;
}
...
bool solve(){
    ...
    // cout<<sum<<nl;
    sp=vector<vector<int>>(m+1,vector<int>(sum+1,0));
    dp=vector<vector<int>>(m+1,vector<int>(sum+1,-1));
    cout<<rec(0,0,0)<<nl;
    // for(int i=sum;i>=0;--i){
    //     if(dp[0][i] > 0){
    //         cout<<i<<nl;
