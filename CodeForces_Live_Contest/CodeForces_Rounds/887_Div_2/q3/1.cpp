...
bool solve(){
    ...
    cin >> n >> k ;
    iv(v , n)
    int minimum_a_not_present = 1 ;
    for(int i = 0 ; i < n ; ++i){
        if(v[i] == minimum_a_not_present){
            minimum_a_not_present += 1 ;
        }else{
            break ;
        }
    }
    int total_a_not_present = v[n - 1] - n ;
    int present_present = n ;
    int divide = total_a_not_present / minimum_a_not_present ;
    if(k == 1){
        cout << minimum_a_not_present << nl ;
        return true ;
    }
    if(v[0] != 1){
        cout << 1 << nl ;
    ...
    if(divide >= k){
        ...
        int rem = total_a_not_present % minimum_a_not_present ;
        int kk = minimum_a_not_present - rem ;
        int kk2 = minimum_a_not_present - (minimum_a_not_present - rem) ;
        int kitna_bacha_hai_bhai = k - divide - 1 ;
        int last = v[n - 1] ;
        int first = kitna_bacha_hai_bhai / 2 ;
        int second = (kitna_bacha_hai_bhai + 1) / 2 ;
        last += (n) * (kitna_bacha_hai_bhai) ;
        cout << divide << " " << kitna_bacha_hai_bhai << " " << kk << nl ;
        int ans = last +  ;
        cout << ans << nl ;
    }
    return true ;
