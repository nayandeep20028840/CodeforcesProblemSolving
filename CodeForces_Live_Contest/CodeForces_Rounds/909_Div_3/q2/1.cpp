...
vector<vector<long long>> givemedivisors(long long n){long long*arr = new long long[n + 1]();vector<vector<long long > > vect(n + 1);for(long long i = 2; i <= n; ++i){if(arr[i] == 0){for(long long j = i; j <= n; j += i){vect[j].pb(i);arr[i] = 1;}}} return vect ;}
