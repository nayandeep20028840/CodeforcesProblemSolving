...
bool solve(){
    ...
    for(int i = 0 ; i < m ; ++i){
        ans = min(ans , (abs(mn - mx) + abs(mx - vv[i])));
    }
    int ans2 = INT_MAX;
    for(int i = 0 ; i < m ; ++i){
        ans2 = min(ans2 , (abs(mx - mn) + abs(mn - vv[i])));
    }
    int ans3 = INT_MAX;
    for(int i = 0 ; i < m ; ++i){
        ans3 = min(ans3 , (abs(mx - vv[i]) + abs(vv[i] - mn)));
    }
    if(max({ans , ans2 , ans3}) != INT_MAX)
    cout << max({ans , ans2 , ans3}) << nl;
    else
    cout << 0 << nl;
    // if(max(ans , ans2) == INT_MAX) cout << 0 << nl;
