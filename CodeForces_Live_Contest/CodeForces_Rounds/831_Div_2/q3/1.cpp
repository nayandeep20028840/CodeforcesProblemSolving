...
bool solve(){
    ...
    int ans = INT_MAX;
    for(int i = 1 ; i < n - 1 ; ++i){
        ans = min(ans , (abs(v[0] - v[n - 1]) + abs(v[n - 1] - v[i])));
    }
    int ans2 = INT_MAX;
    for(int i = 1 ; i < n - 1 ; ++i){
        ans2 = min(ans2 , (abs(v[n - 1] - v[0]) + abs(v[0] - v[i])));
    }
    int ans3 = INT_MAX;
    for(int i = 1 ; i < n - 1 ; ++i){
        ans3 = min(ans3 , (abs(v[n - 1] - v[i]) + abs(v[i] - v[0])));
    }
    // int ans4 = INT_MAX;
    // for(int i = 1 ; i < n - 1 ; ++i){
    //     ans4 = min(ans4 , (abs(v[n - 1] - v[0]) + abs(v[0] - v[i])));
    // }
