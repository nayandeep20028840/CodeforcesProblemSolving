...
bool solve(){
    ...
    int ans = INT_MIN;  
    // if(len <= 2){
    //     ans = max(abs(v[0] - v[n - 1]) + abs(v[n - 1] - v[1]) , abs(v[0] - v[n - 1]) + abs(v[n - 1] - v[n - 2]));
    //     ans = max(abs(v[0] - v[n - 1]) + abs(v[0] - v[1]) , abs(v[0] - v[n - 1]) + abs(v[0] - v[n - 2]));
    // }
    while(low < k){
        ans = max(ans , abs(vv[low] - vv[high]) + abs(vv[high] - vv[k]));
        high--;
        k--;
    }
    high = len - 1 ;
    ...
    int ans2 = INT_MIN;
    while(high > k){
        ans2 = max(ans2 , abs(vv[high] - vv[low]) + abs(vv[low] - vv[k]));
        low++;
        k++;
    }
    if(max(ans , ans2) == INT_MIN) cout << 0 << nl;
    else cout << max(ans , ans2) << nl;
    return true;
}
