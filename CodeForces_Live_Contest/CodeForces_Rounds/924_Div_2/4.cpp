...
bool solve(){
    ...
    2 2 2 2 2 2 2
    t t t t t t t = 7 * t ;
    t * (6 * t) + t * (5 * t) + t * (4 * t) + t * (3 * t) + t * (2 * t) + t * (1 * t) ;
    t * t * (6 + 5 + 4 + 3 + 2 + 1)
    t * t * (n * (n + 1) / 2)
    ...
    2 1
    */
    vector<int > cnt(mx + 10) ;
    vector<int > answer(mx + 10) ;
    for(int i = 0 ; i < n ; i++){
        int ans = 0 ;
        for(int j = 1 ; j <= c[i] ; ++j){
            ans = 0 ;
            ...
            int rem = c[i] % j ;
            if(rem == 0){
                ans += times * times * ((j * (j - 1)) / 2) ;
            }
            else{
                times += 1 ;
                ans += times * times * (j * (j - 1) / 2) ;
                ans -= times * times * ((j - rem) * (j - rem - 1) / 2) ;
                ans -= times * rem * (j - rem) * times ;
                times -= 1 ;
                ans += times * rem * (j - rem) * (times + 1) ;
                ans += times * times * ((j - rem) * (j - rem - 1) / 2) ;
            }
            // cout << c[i] << " " << j << " " << times << " " << rem << " " << ans << nl ;
            answer[j] += ans ;
        }
        cnt[c[i] + 1] += ans ;
    }
