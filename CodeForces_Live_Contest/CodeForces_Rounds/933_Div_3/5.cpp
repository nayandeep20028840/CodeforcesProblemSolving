...
public:
    ...
    T at(int idx) {
        // assert(0 <= idx and idx < n);
        return min(query(idx) , (idx - 1 >= 0 ? query(idx - 1) : inf));
    }

    // sum of all the element [l..r]
    T at(int l, int r) {
        // assert(0 <= l and l <= r and r < n);
        return min(query(r) , (l - 1 >= 0 ? query(l - 1) : inf));
    }
};
...
bool solve(){
    ...
    for(int i=0;i < n ; ++i){
        FT<int>ft(m);
        // cout<<st.make_query(0,n).val<<" " ;
        int mn=0;
        // for(int j = 0 ; j < m ; ++j){
        //     cout<<st.make_query(j,j).val<<" ";
        // }
        for(int j = 0  ; j < m ;  ++j){
            // cout<<mat[i][j]<<" ";
            // if(j==0)continue;
            mn = ft.at(max(0ll,j-d-1),max(0ll,j-1));
            ft.inc(j,mat[i][j]+1+mn);
            // cout<<mn<<" " ;
            // cout<<mat[i][j]+1+mn<<" ";
            // cout<<ft.at(j,j)<<" ";
        }
    ...
    //     for(int j=0;j<m;++j){
    //         if(mt.size()>d+1){
    //             mt.erase(mt.find(cur[it]));
    //             // mp[it]-=1;
    //             it+=1;
    //         }
    //         int cal=0;
    //         if(mt.size()) cal = *(mt.begin());
    //         // cout<<cal<<" ";
    //         // cout<<mt.size()<<" ";
    //         cur[j] = mat[i][j]+1+cal;
    //         // cout<<cur[j]<<" ";
