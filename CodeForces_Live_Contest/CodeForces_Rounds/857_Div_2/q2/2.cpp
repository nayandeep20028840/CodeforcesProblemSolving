...
bool solve(){
    ...
    for(int i = 0 ; i < n ; ++i){
        if(v[i] == 1){
            ...
            if(female_boxes > male_boxes){
                if(male_boxes % 2 == 0){
                    male_boxes = (cnt + 1) / 2 ;
                    female_boxes = cnt / 2 ;
                }
                else if(female_boxes % 2 == 0){
                    male_boxes = cnt / 2 ;
                    female_boxes = (cnt + 1) / 2 ;
                }
            }
            else if(female_boxes == male_boxes){
                female_boxes = (cnt + 1) / 2 ;
                male_boxes = cnt / 2 ;
            }
            empty_boxes = total_boxes - (female_boxes + 1) / 2 - (male_boxes + 1) / 2 ;
        }
        if(v[i] == 2 && cnt > 2){
        }
    }
