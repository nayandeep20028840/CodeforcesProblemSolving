...
bool solve(){
    ...
    vector<int > answer(max(k + 1 , n + 1) , 0) ;
    vector<int > min_pos(max(k + 1 , n + 1) , 0) ;
    vector<int > max_pos(max(k + 1 , n + 1) , 0) ;
    map<int , int > mp ;
    for(int i = 0 ; i < n ; ++i){
        if(mp[a[i]] == 0){
            mp[a[i]] = i + 1 ;
    ...
    for(int i = k ; i > 0 ; --i){
        if(mx >= max_pos[i] && min_pos[i] != 0){
            ...
            mn2 = min({mn2 , min_pos[i] , max_pos[i]}) ;
            mx = max({mx , min_pos[i] , max_pos[i]}) ;
        }
        if(mn2 < min_pos[i] && min_pos[i] != 0){
            min_pos[i] = mn2 ;
        }
        else{
            if(min_pos[i] == 0){
    ...
    for(int i = 1 ; i <= k ; ++i){
        if(mn == i){
            ...
            answer[i] = (max_pos[i] - min_pos[i] + 1) * 2 ;
        }
    }
    for(int i = 1 ; i <= k ; ++i){
        cout << answer[i] << " " ;
